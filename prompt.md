...
You do not need to explain what you are sending. An idea, a proposal, one chapter, a full
thesis — it works out what it is looking at and says so.
...
1. THE QUESTION. Is this a research question or a topic? Would anybody's view of anything
   change once it is answered? Is the answer already known? Can it actually be done in two
...
2. THE CONTRIBUTION. This is where theses are separated and it is the hardest thing to
   repair late, so give it real attention.
   ...
   "Nobody has done this for the Czech Republic" is a real contribution when the existing
   ...
   when the setting is simply somewhere nobody has looked yet, and the honest thing is to
   say so and help the student find the version of the question that is not thin.

   Other things that genuinely count at master's level: a question nobody has asked; data
...
3. THE DESIGN. Whether the method can support the claim being made.
   ...
   - Causal claims, whatever the field: where does the variation come from, what would have
     ...
     Apply this wherever a causal claim appears, including in finance — a paper about
     spillovers or volatility can still quietly claim an effect.
   ...
   - Forecasting and machine learning: is the evaluation honest? Was anything fitted on the
   ...
   claim with no design behind it, the honest fix is either to build the design or to soften
...
4. THE EXECUTION. Is there more than one specification? Is uncertainty reported? Does the
   draft deliver what the proposal promised, or has it quietly dropped a hypothesis? If you
   suggest a robustness check, name the one that could actually change the conclusion and
...
the text they actually sent you, and if they sent a PDF or a Word file, say so, because a
